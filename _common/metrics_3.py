...
# average transpile factor between 1 QV depth and our depth based on results from QV notebook
    ...
    try:       
        ...
        if aq_mode > 0:
            ...
            ax = plot_volumetric_background(max_qubits=max_qubits, QV=QV, depth_base=depth_base,
                suptitle=suptitle, avail_qubits=avail_qubits, colorbar_label="Success Probability")
...
def plot_volumetric_background(max_qubits=11, QV=32, depth_base=2, suptitle=None, avail_qubits=0, colorbar_label="Avg Result Fidelity", ax=None):
    ...
    log2QV = math.log2(QV)
    QV_width = 0
    QV_depth = log2QV * (QV_transpile_factor+ 33)
    #QV_depth = log2QV * 10e5
    ...
    plt.colorbar(cm.ScalarMappable(cmap=cmap), cax=None, ax=ax,
            shrink=0.6, label=colorbar_label, panchor=(0.0, 0.7)).set_label(colorbar_label, fontsize=18)
